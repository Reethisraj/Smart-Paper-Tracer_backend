...
def analyze():
    ...
    if "answer_key" not in request.files:
        return jsonify({"error": "answer_key missing"}), 400

    if "student_sheet" not in request.files:
        return jsonify({"error": "student_sheet missing"}), 400

    answer_key_file = request.files["answer_key"]
    student_file = request.files["student_sheet"]

    answer_path = os.path.join(UPLOAD_FOLDER, answer_key_file.filename)
    ...
    result = compare_answers(answer_text, student_text)
